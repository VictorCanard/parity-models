...
from datasets.code_dataset import BATCH_SIZE
...
class GPTBase(nn.Module):
    ...
    def forward(self, idx, targets=None):
        sum_emb = self.forward1(idx)
        return self.forward2(sum_emb, targets=targets)
    ...
    def forward2(self, sum_emb, targets=None):
        x = self.transformer.drop(sum_emb)
        for block in self.transformer.h:
            x = block(x)
        x = self.transformer.ln_f(x)

        if targets is not None or True:
