...
```bash
docker run -it --rm -v /mnt/c/Users/victo/OneDrive/Documents/Etudes/BSc/SIL/parity-models:/workspace/parity-models parity-models-pytorch:latest
```
3. Start training the MNIST parity model:
```bash
# These commands should be run from within the Docker container
cd /parity-models/train
python3 train.py config/mnist/mnist-concat.json save
```
...
previous run with:
```bash
python3 train.py config/mnist-concat.json save --continue_from_file save/mnist/base-mlp/k2/torch.nn.MSELoss/coders.image.ConcatenationEncoder/coders.summation.SubtractionDecoder/current.pth
```
